...
FROM customers AS c
LEFT JOIN public.accounts AS a 		ON c.customer_id = a.customer_id
LEFT JOIN public.transactions AS t 	ON a.account_id = t.account_id
GROUP BY c.customer_id, c.first_name, c.last_name
...
-- Your answer here
WITH branch_total_transaction AS
( SELECT b.branch_id,
         b.branch_name,
         COUNT(t.transaction_id) AS total_transactions
  FROM branches AS b
  LEFT JOIN accounts AS a 		ON b.branch_id = a.branch_id
...
SELECT branch_id,
	   branch_name,
	   total_transactions
FROM branch_total_transaction
WHERE total_transactions = (SELECT MAX(total_transactions) FROM branch_total_transaction)
ORDER BY branch_id;
