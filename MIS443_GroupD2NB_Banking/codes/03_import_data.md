...
## Alternative: load the data with INSERT

If you would rather not import the CSV files, run the statements below in the Query Tool
(connected to `banking`, after file 02). Same rule on order: a row can only point at a row
that already exists, so branches go in first and transactions/loans last. Run this block
once; a second run fails on the duplicate primary keys.
